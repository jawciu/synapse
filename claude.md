...
- Chat and reflection pipelines now verify the shared SurrealDB connection is alive before use (`_ensure_live_connection()` in [reflect/service.py](reflect/service.py) runs a cheap `RETURN 1` through the reconnect helper). This fixed the prod bug where SurrealDB Cloud closed the idle websocket and every chat died mid-tool-call with `no close frame received or sent`, which the UI rendered as "No response returned."
- The chat and reflection SSE streams now emit a `{"type": "error"}` event when the agent fails mid-run (instead of terminating silently), and the frontend surfaces that error to the user in both the talk transcript and the reflection composer.
- Non-streaming `/api/chat` no longer 500s when agent history contains tool-use content blocks: message content is flattened to plain text via `_content_to_text()` before `AskResponse` validation.
...
- A minimal `pytest` smoke suite now exists under `tests/test_smoke.py` covering auth utility roundtrips, source/thread normalization helpers, and message-content flattening (`_content_to_text`). Run it with `uv run python -m pytest tests/` — plain `uv run pytest` fails with `ModuleNotFoundError: No module named 'reflect'` because there is no conftest/pythonpath config putting the repo root on `sys.path`.
...
- All entity upserts in [reflect/graph_store.py](reflect/graph_store.py) now normalize names with `.strip().lower()` before lookup/storage (previously `theme`, `emotion`, and `body_signal` stored raw casing, which silently created duplicate records like `Past bleeding into present` vs `past bleeding into present`). Person names continue to use `.strip().title()` for display readability.
