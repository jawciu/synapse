...
- A minimal `pytest` smoke suite now exists under `tests/test_smoke.py` covering auth utility roundtrips, source/thread normalization helpers, and message-content flattening (`_content_to_text`). Run it with `uv run pytest` — `[tool.pytest.ini_options]` in [pyproject.toml](pyproject.toml) sets `pythonpath = ["."]` and `testpaths = ["tests"]` so the `reflect` package resolves from the repo root.
